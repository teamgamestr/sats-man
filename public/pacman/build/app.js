class Ghost {
  ...
  setSpriteSheet(name, direction, mode) {
    ...
    if (mode === 'scared') {
      this.animationTarget.style.backgroundImage = 'url(/pacman/app/style/graphics/'
...
class Pacman {
  ...
  reset() {
    ...
    this.pacmanArrow.style.backgroundImage = 'url(/pacman/app/style/graphics/'
  ...
  prepDeathAnimation() {
    ...
    this.animationTarget.style.backgroundSize = `${bgSize}px`;
    this.animationTarget.style.backgroundImage = 'url(/pacman/app/style/'
...
class GameCoordinator {
  ...
  setSoundButtonIcon(newVolume) {
    this.soundButton.className = `material-icons satsman-control-icon ${newVolume === 0 ? 'satsman-volume-off-icon' : 'satsman-volume-on-icon'}`;
  }
  ...
  preloadAssets() {
    return new Promise((resolve) => {
      ...
      const imgBase = '/pacman/app/style/graphics/spriteSheets/';
      const imgSources = [
        // Pacman
        ...
        // Misc
        '/pacman/app/style/graphics/extra_life.svg',
      ];

      const audioBase = '/pacman/app/style/audio/';
      const audioSources = [
  ...
  updateExtraLivesDisplay() {
    ...
    for (let i = 0; i < this.lives; i += 1) {
      ...
      extraLifePic.setAttribute('src', '/pacman/app/style/graphics/extra_life.svg');
  ...
  handlePauseKey() {
    if (this.allowPause) {
      ...
      if (this.gameEngine.started) {
        ...
        this.pauseButton.className = 'material-icons satsman-control-icon satsman-pause-icon';
        this.activeTimers.forEach((timer) => {
          timer.resume();
        });
      } else {
        ...
        this.pauseButton.className = 'material-icons satsman-control-icon satsman-play-icon';
        this.activeTimers.forEach((timer) => {
          timer.pause();
        });
      }
   ...
   */
  gameOver() {
    window.dispatchEvent(new CustomEvent('satsman:pacman-game-over', { detail: { score: this.points, highScore: this.highScore, level: this.level } }));
    localStorage.setItem('highScore', this.highScore);
  ...
  advanceLevel() {
    ...
    const imgBase = '/pacman/app/style/graphics/spriteSheets/maze/';
  ...
  displayText(position, amount, duration, width, height) {
    ...
    pointsDiv.style.backgroundImage = 'url(/pacman/app/style/graphics/'
...
class Pickup {
  ...
  determineImage(type, points) {
    ...
    return `url(/pacman/app/style/graphics/spriteSheets/pickups/${image}.svg)`;
  }
...
class SoundManager {
  constructor() {
    this.baseUrl = '/pacman/app/style/audio/';
    this.fileFormat = 'mp3';
    this.masterVolume = 1;
    this.paused = false;
    this.cutscene = true;
    ...
    this.ambience = window.satsmanAudioContext || new AudioContext();

    // Dedicated AudioContext for dot sounds (mobile optimization)
    this.dotContext = window.satsmanDotAudioContext || new AudioContext();
  ...
  play(sound) {
    ...
    fetch(`${this.baseUrl}${sound}.${this.fileFormat}`)
      .then((response) => response.arrayBuffer())
      .then((arrayBuffer) => this.ambience.decodeAudioData(arrayBuffer))
      .then((audioBuffer) => {
        const source = this.ambience.createBufferSource();
        const gain = this.ambience.createGain();
        gain.gain.value = this.masterVolume;
        source.buffer = audioBuffer;
        source.connect(gain);
        gain.connect(this.ambience.destination);
        source.start();
        this.soundEffect = source;
      })
      .catch(() => {
