...
export function useLoginActions() {
  ...
  return {
    ...
    anonymous(aliasPubkey?: string, metadata?: { identifier?: string; source?: string }): string {
      const secretKey = generateSecretKey();
      const pubkey = getPublicKey(secretKey);
      const login = NLogin.fromNsec(nip19.nsecEncode(secretKey));

      if (aliasPubkey || metadata) {
        sessionStorage.setItem(`satsman:alias:${pubkey}`, JSON.stringify({
          aliasPubkey,
          nip05: metadata?.identifier,
          loginSource: metadata?.source ?? 'anonymous',
        }));
      }
